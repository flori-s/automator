...
- `c.email_delivery` callable for host/custom mailers that do not match Automator's mailer method API (e.g. existing `DocumentMailer`-style senders). Receives a message hash (`to`, `subject`, `vars`, `record`, `payload`, …) and is responsible for delivery.
...
[0.1.2]: https://github.com/flori-s/automator/releases/tag/v0.1.2
